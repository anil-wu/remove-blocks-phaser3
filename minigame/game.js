...
const config = {
    type: Phaser.CANVAS, // 小游戏推荐使用 CANVAS 模式，或 WEBGL
    width: windowWidth,
    height: windowHeight,
    backgroundColor: '#2d2d2d',
    canvas: GameGlobal.canvas || wx.createCanvas(), // 微信小游戏的全局 canvas 对象
    scene: {
        preload: preload,
        create: create,
        update: update
...
const TILE_SIZE = Math.floor(windowWidth / 9); // 自适应方块大小
const ROWS = 8;
const COLS = 8;
const OFFSET_X = (windowWidth - COLS * TILE_SIZE) / 2 + TILE_SIZE / 2;
const OFFSET_Y = (windowHeight - ROWS * TILE_SIZE) / 2 + TILE_SIZE / 2;
